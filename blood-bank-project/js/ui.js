...
function showToast(message, type = 'success') {
  ...
  const icon = type === 'success' ? Icons.check : type === 'error' ? Icons.x : Icons.alertCircle;
...
function renderSidebar(activePage) {
  ...
  return `
    <div class="sidebar-brand">
      <div class="brand-icon">${Icons.droplet}</div>
      <div>
        <h2>BloodLink</h2>
        <span style="display:block">Blood Bank System</span>
      </div>
    </div>
    <nav class="sidebar-nav">
      ...
      <div class="nav-section">
        ...
        <a href="donors.html" class="nav-item ${activePage === 'donors' ? 'active' : ''}">${Icons.users}<span>Donors</span></a>
        <a href="inventory.html" class="nav-item ${activePage === 'inventory' ? 'active' : ''}">${Icons.package}<span>Inventory</span></a>
        <a href="requests.html" class="nav-item ${activePage === 'requests' ? 'active' : ''}">${Icons.clipboard}<span>Blood Requests</span></a>
        <a href="camps.html" class="nav-item ${activePage === 'camps' ? 'active' : ''}">${Icons.tent}<span>Donation Camps</span></a>
      </div>
    </nav>
    <div class="sidebar-footer">
      ...
      <button class="btn btn-ghost w-full" style="margin-top:8px;justify-content:center;" onclick="Auth.logout()">
...
function renderTopbar(title) {
  return `
    <div class="topbar-left">
      ...
      <span class="text-sm text-muted">${new Date().toLocaleDateString('en-IN', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}</span>
    </div>
    <div class="topbar-right">
      <button class="notif-btn" title="Notifications">${Icons.bell}<span class="notif-dot"></span></button>
    </div>
  `;
}
...
function urgencyBadge(urgency) {
  const map = {
    ...
    normal: { class: 'badge-green', label: 'Normal' }
  };
...
function statusPill(status) {
  ...
  return `<span class="status-pill ${map[status] || 'status-pending'}">${status.charAt(0).toUpperCase() + status.slice(1)}</span>`;
}

function stockLevelBadge(units) {
  if (units <= 2) return `<span class="badge badge-red">Critical</span>`;
  if (units <= 5) return `<span class="badge badge-amber">Low</span>`;
  return `<span class="badge badge-green">Good</span>`;
}
...
function showLoading(containerId) {
  ...
  el.innerHTML = `<div style="padding:40px;text-align:center;color:var(--gray-400)">
    <div class="skeleton" style="width:60%;height:16px;margin:0 auto 12px"></div>
    <div class="skeleton" style="width:40%;height:12px;margin:0 auto"></div>
  </div>`;
}